    ...
    public partial class MainWindow : Form
    {
        ...
        private void MainWindow_Load(object sender, EventArgs e)
        {
            Log.Append("Slim Updater v" + ProductVersion + " " + "started on " +
                Utilities.GetFriendlyOSName(), Log.AppendLevel.INFO, logTextBox);
        }
        ...
        {
            Log.Append("Loading definitions file", Log.AppendLevel.INFO, logTextBox);
            ...
            try
            {
                if (Settings.DefenitionURL != null)
                {
                    Log.Append("Using custom definition file from " + Settings.DefenitionURL,
                        Log.AppendLevel.INFO, logTextBox);
                    defenitions = XDocument.Load(Settings.DefenitionURL);
                }
                else
                {
                    Log.Append("Using official definitions", Log.AppendLevel.INFO, logTextBox);
            ...
            catch (Exception e)
            {
                Log.Append("Cannot check for updates: " + e.Message, 
                    Log.AppendLevel.ERROR, logTextBox);
        ...
        public bool CheckForUpdates()
        {
            ...
            Log.Append("Checking for updates...", Log.AppendLevel.INFO, logTextBox);
            ...
            else
            {
                ...
                Log.Append("No updates available", Log.AppendLevel.INFO, logTextBox);
        ...
        public async void InstallUpdates(List<App> updateList)
        {
            ...
            Log.Append("Update started...", Log.AppendLevel.INFO, logTextBox);
            ...
            if (selectedUpdateList.Count == 0)
            {
                ...
                Log.Append("No updates selected to install, aborting...",
                    Log.AppendLevel.WARN, logTextBox);
            }
            ...
            foreach (App update in selectedUpdateList)
            {
                ...
                Task downloadTask = Task.Run(async () =>
                {
                    ...
                    Log.Append("Saving to: " + update.SavePath, Log.AppendLevel.INFO, logTextBox);
                        ...
                        using (var wc = new WebClient())
                        {
                            ...
                            catch (Exception e)
                            {
                                ...
                                Log.Append("An error occurred when attempting to download " +
                                    "the update." + e.Message, Log.AppendLevel.ERROR, logTextBox);
                                if (InvokeRequired)
                                {
                                    Invoke(new MethodInvoker(() =>
                                    {
            ...
            foreach (App update in selectedUpdateList)
            {
                ...
                if (File.Exists(update.SavePath))
                {
                    Log.Append(string.Format("Installing {0} ({1} of {2}) ...", update.Name,
                        currentUpdate, selectedUpdateList.Count), Log.AppendLevel.INFO, logTextBox);
                    using (var p = new Process())
                    {
                        ...
                        catch (Exception)
                        {
                            ...
                            if (result == DialogResult.Yes)
                            {
                                Log.Append("User chose yes.", Log.AppendLevel.INFO, logTextBox);
                                goto launchInstaller;
                            }
                            else
                            {
                                Log.Append("User chose no. Skipping this update.", Log.AppendLevel.INFO, logTextBox);
                                continue;
                            }
                        }
                        ...
                        if (p.ExitCode != 0)
                        {
                            Log.Append(string.Format("Installation failed. Installer exited with " +
                                "exit code {0}.", p.ExitCode), Log.AppendLevel.ERROR, logTextBox);
                            update.AppItem.Status = string.Format(
            ...
            {
                Log.Append("Cleaning up leftover installers...", Log.AppendLevel.INFO, logTextBox);
        ...
        public async void InstallNewApps(List<App> appList)
        {
            ...
            Log.Append("New app installation started...", Log.AppendLevel.INFO, logTextBox);
            ...
            foreach (App app in selectedAppList)
            {
                ...
                Task downloadTask = Task.Run(async () =>
                {
                    ...
                    Log.Append("Saving to: " + app.SavePath, Log.AppendLevel.INFO, logTextBox);
                    ...
                    if (!File.Exists(app.SavePath))
                    {
                        using (var wc = new WebClient())
                        {
                            ...
                            {
                                Log.Append("An error occurred when attempting to download " +
                                    "the installer." + e.Message, Log.AppendLevel.ERROR, logTextBox);
                                if (InvokeRequired)
                                {
                                    Invoke(new MethodInvoker(() =>
                                    {
            ...
            foreach (App app in selectedAppList)
            {
                ...
                if (File.Exists(app.SavePath))
                {
                    Log.Append(string.Format("Installing {0} ({1} of {2}) ...", app.Name,
                        currentUpdate, selectedAppList.Count), Log.AppendLevel.INFO, logTextBox);
                    using (var p = new Process())
                    {
                        ...
                        catch (Exception)
                        {
                            ...
                            {
                                Log.Append("User chose no. Skipping this app.", Log.AppendLevel.INFO, logTextBox);
                                continue;
                            }
                        }
        ...
        public async void InstallPortableApps(List<PortableApp> portableAppList, bool runOnce)
        {
                ...
                Task downloadTask = Task.Run(async () =>
                {
                    ...
                    if (!File.Exists(app.SavePath))
                    {
                        using (var wc = new WebClient())
                        {
                            ...
                            {
                                Log.Append("An error occurred when attempting to download " +
                                    "the Portable App." + e.Message, Log.AppendLevel.ERROR, logTextBox);
                                if (InvokeRequired)
                                {
                                    Invoke(new MethodInvoker(() =>
                                    {
            ...
            Log.Append("7-Zip path: " + sevenZipPath, Log.AppendLevel.INFO, logTextBox);
            ...
            foreach (PortableApp app in selectedAppList)
            {
                if (File.Exists(app.SavePath))
                {
                    ...
                    {
                        Log.Append("Launching " + app.Name, Log.AppendLevel.INFO, logTextBox);
                        ...
                        Log.Append("All processes exited. Cleaning up...", Log.AppendLevel.INFO, logTextBox);
                    ...
                    if (app.ExtractMode == "folder")
                    {
                        using (var p = new Process())
                        {
                            ...
                            if (p.ExitCode == 0)
                            {
                                Log.Append("Extract succesful.", Log.AppendLevel.INFO, logTextBox);
                                File.Delete(app.SavePath);
                                app.AppItem.Status = "Install complete";
                                app.AppItem.Progress = 100;
                                app.AppItem.ProgressBarStyle = ProgressBarStyle.Continuous;
